...
export function ensureLegacyServices(catalog:AgencyService[],records:ServiceLinkedRecord[]){const next=[...catalog];for(const reference of records.flatMap(record=>record.serviceIds?.length?record.serviceIds:record.services||[])){if(next.some(service=>service.id===reference||normalize(service.name)===normalize(reference)))continue;let id=`legacy-${slug(reference)}`,suffix=2;while(next.some(service=>service.id===id))id=`legacy-${slug(reference)}-${suffix++}`;next.push({id,name:reference,description:'Serviço preservado durante a migração dos vínculos existentes.',price:0,pricingType:'variable',capabilities:normalize(reference)==='social media'?['editorial']:[],active:true,createdAt:new Date().toISOString()})}return next}
export const hasEditorialCapability=(service:AgencyService|undefined)=>Boolean(service?.capabilities?.includes('editorial'));
export function migrateServiceCapabilities(catalog:AgencyService[]):AgencyService[]{return catalog.map(service=>{if(hasEditorialCapability(service))return service;const wasSocialMedia=service.id==='social'||normalize(service.name)==='social media';return wasSocialMedia?{...service,capabilities:['editorial'] as ServiceCapability[]}:service})}
