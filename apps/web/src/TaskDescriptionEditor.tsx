...
export default function TaskDescriptionEditor({defaultValue='',clientId,attachments,onAttachmentsChange,onUploaded,hideLabel=false}:{defaultValue?:string;clientId:string;attachments:TaskAttachment[];onAttachmentsChange:(items:TaskAttachment[])=>void;onUploaded?:(fileId:string)=>void;hideLabel?:boolean}){
 ...
 const applyChange=(next:{value:string;selectionStart:number;selectionEnd:number})=>{const element=textarea.current;setValue(next.value);requestAnimationFrame(()=>{element?.focus();element?.setSelectionRange(next.selectionStart,next.selectionEnd)})};
 const format=(marker:string,endMarker=marker)=>{const element=textarea.current;if(element)applyChange(wrapDescriptionSelection(value,element.selectionStart,element.selectionEnd,marker,endMarker))};
 const addChecklist=()=>{const element=textarea.current;if(element)applyChange(insertDescriptionChecklist(value,element.selectionStart,element.selectionEnd))};
 ...
 return <div className="taskDescriptionEditor full">
  ...
  <div className="taskDescriptionBox">
   <div className="taskDescriptionToolbar" role="toolbar" aria-label="Formatação da descrição">
    <div className="taskToolbarGroup"><button type="button" aria-label="Negrito" title="Negrito (Ctrl+B)" onClick={()=>format('**')}><Bold/></button><button type="button" aria-label="Itálico" title="Itálico (Ctrl+I)" onClick={()=>format('_')}><Italic/></button><button type="button" aria-label="Tachado" title="Tachado (Ctrl+Shift+S)" onClick={()=>format('~~')}><Strikethrough/></button><button type="button" className={showColors?'active':''} aria-label="Cor do texto" title="Cor do texto" onClick={()=>setShowColors(current=>!current)}><Palette/></button><button type="button" aria-label="Adicionar checklist" title="Adicionar checklist" onClick={addChecklist}><ListChecks/></button></div>
    <i/>
   ...
   {showColors&&<div className="taskColorPicker"><span>Cor do texto</span>{textColors.map(color=><button key={color.value} type="button" title={color.label} aria-label={color.label} style={{backgroundColor:color.value}} onClick={()=>applyColor(color.value)}/>)}<label title="Cor personalizada"><input aria-label="Escolher cor personalizada" type="color" value={customColor} onChange={event=>setCustomColor(event.target.value)}/></label><button type="button" className="applyCustomColor" onClick={()=>applyColor(customColor)}>Aplicar</button></div>}
   <textarea ref={textarea} name="description" value={value} onChange={event=>setValue(event.target.value)} onKeyDown={handleKeyDown} placeholder="Escreva o contexto, briefing, instruções e demais detalhes desta tarefa..."/>
