...
export default function Sidebar({open,member,onNavigate}:{open:boolean;member?:TeamMember;onNavigate:()=>void}){
 ...
 return <aside id="app-sidebar" className={`sidebar ${open?'open':'closed'}`}>
  ...
  <nav>{navGroups.map(group=>{
   const items=group.items.filter(item=>canAccessPath(member,`/${item.path}`));
