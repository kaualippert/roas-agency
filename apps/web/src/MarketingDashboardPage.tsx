...
import {defaultMarketingMetricIds,formatMarketingMetric,marketingMetricCatalog,metricValue,normalizeMarketingDashboardPreferences,type MarketingDashboardPreference,type MarketingDashboardWidget,type MarketingMetricKey} from './marketing-dashboard-config';
import MarketingDashboardWidgets from './MarketingDashboardWidgets';
...
export default function MarketingDashboardPage(){
 ...
 const clientPreference=preferences.find(item=>item.clientId===clientId),metricIds=clientPreference?.metricIds||defaultMarketingMetricIds,widgets=clientPreference?.widgets||[];
 ...
 const savePreference=(nextMetricIds:MarketingMetricKey[],nextWidgets:MarketingDashboardWidget[])=>{if(!clientId)return;const next:MarketingDashboardPreference={clientId,metricIds:nextMetricIds,widgets:nextWidgets,updatedAt:new Date().toISOString()};setPreferences([next,...preferences.filter(item=>item.clientId!==clientId)])};
 const saveCustomization=()=>{if(!draftMetrics.length)return;savePreference(draftMetrics,widgets);setCustomizing(false)};
 const saveWidgets=(nextWidgets:MarketingDashboardWidget[])=>savePreference(metricIds,nextWidgets);
 ...
 return <main className="marketingDashboardPage marketingClientDashboard">
  ...
  {!visibleSnapshots.length?<section className="card marketingNoData"><BarChart3/><h3>Este canal ainda não possui dados</h3><p>Volte às integrações e sincronize a conta de anúncios deste cliente.</p><Link className="btn" to="/marketing/integrations">Sincronizar conta</Link></section>:<>
   ...
   <MarketingDashboardWidgets metrics={metrics} widgets={widgets} onChange={saveWidgets}/>
  </>}
