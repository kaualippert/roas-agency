...
export const store={
 ...
 async init(){if(hydrated)return;const [result,accessResult]=await Promise.all([apiRequest('/state'),apiRequest('/access/me')]);state=result.state||{};if(state.general_settings)cacheLoadingBrand(state.general_settings);currentAccess=accessResult.access||null;hydrated=true;emit('hydrate')},
 clearSession(){hydrated=false;state={};currentAccess=null;clearPending();emit('hydrate')},
 snapshot(){return structuredClone(state)},
 async replaceAll(next:Record<string,unknown>){clearPending();const result=await apiRequest('/state',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({state:next})});state=result.state||{};if(state.general_settings)cacheLoadingBrand(state.general_settings);hydrated=true;emit('hydrate')},
